...
- [ ] **Phase 5 — Madgwick fusion per IMU** (`firmware/05_madgwick_fusion/`) 🟡 bench-verified, closing 2026-07-17
  ...
  Second pass adds accel-seeded initial orientation and a two-stage β. Bench-tested: gyro bias std now 0.05-0.35 deg/s (genuinely still calibration), no sensor dropouts, roll/pitch drift ≤ ~1.7 deg/min, yaw drift ~0.2-2.1 deg/min (expected, unbounded without a magnetometer). Still needed before closing: a same-session bias-on/off A/B pair and a multi-minute drift run (current one is ~31.5 s).
...
- [x] **Phase 7 — Full glove mount** (`firmware/07_full_glove/`) ✅ 2026-07-18
